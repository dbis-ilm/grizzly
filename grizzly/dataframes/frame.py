...
class DataFrame(object):
  ...
  def _exec_or_add_aggr(self, col, aggFunc, alias):
    ...
    """
    isGroupCol = col.column in [c.column for c in self.groupCols]
    if isinstance(self, Grouping) and aggFunc and not isGroupCol:
      self.aggFunc.append((aggFunc, col, alias))
      return self
...
class Grouping(DataFrame):
  ...
  def filter(self, expr):
    # TODO expr should contain references to projections (the computed aggregate) -> 
    # remove tuple variable / qualifiers
