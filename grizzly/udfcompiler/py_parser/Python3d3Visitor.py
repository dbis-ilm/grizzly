...
class Python3d3Visitor(ParseTreeVisitor):
    ...
    def visitNontype_initialization(self, ctx:Python3d3Parser.Nontype_initializationContext):
        ...
        if var not in self.assignments:
            ...
            if declr_expr.typecast():
                ...
                var_type = self.templates[var_type_raw]
            ...
            # If expression is a list try to load mapped datatype
                ...
                if list_expr.elems():
                    ...
                    if first_elem.NUMBER():
                        ...
                        var_type = var_type.replace('$$datatype$$', self.templates['str'])
                    elif first_elem.FLOAT():
                        var_type = var_type.replace('$$datatype$$', self.templates['float'])
                    elif first_elem.BOOL():
                        var_type = var_type.replace('$$datatype$$', self.templates['bool'])
                    else: 
            ...
            # If assignment contains an operation
                ...
                var_type = self.templates[self.assignments[assignment]]
    ...
    def visitExpr(self, ctx: Python3d3Parser.ExprContext):
        ...
        if ctx.calc_op():
            ...
            left_side_contains_str = (ctx.expr()[0].NAME() and self.assignments.get(str(ctx.expr()[0].NAME())) == self.templates['str']) or ctx.expr()[0].STRING()
            right_side_contains_str = (ctx.expr()[1].NAME() and self.assignments.get(str(ctx.expr()[1].NAME())) == self.templates['str']) or ctx.expr()[1].STRING()
            ...
            if left_side_contains_str or right_side_contains_str \
                ...
                or (('CAST' in l or 'CAST' in r) and self.templates['str'] in l or self.templates['str'] in r):
